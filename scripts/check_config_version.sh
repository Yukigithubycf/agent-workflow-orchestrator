...
#
# The chart's `config:` block in deploy/helm/AstraFlow/values.yaml embeds a
...
VALUES_YAML="$ROOT/deploy/helm/AstraFlow/values.yaml"
...
if [ "$chart" -lt "$example" ]; then
  echo "::error::chart config_version ($chart) is behind config.example.yaml ($example). Bump 'config_version' in deploy/helm/AstraFlow/values.yaml (and the README example) to $example." >&2
  exit 1
fi
