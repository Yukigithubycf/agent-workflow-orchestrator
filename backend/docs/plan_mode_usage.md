...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in AstraFlow.
...
AstraFlow uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall AstraFlow prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with AstraFlow's main prompt
...
- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom AstraFlow-style prompts**
...
- Custom prompts emphasize the same principles as AstraFlow's main system prompt (clarity, action-oriented, critical rules)
