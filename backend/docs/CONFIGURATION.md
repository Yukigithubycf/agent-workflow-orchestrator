...
This guide explains how to configure AstraFlow for your environment.
...
- On macOS, AstraFlow does not probe Keychain automatically. Use `scripts/export_claude_code_oauth.py` to export Claude Code auth explicitly when needed
...
AstraFlow supports multiple sandbox execution modes. Configure your preferred mode in `config.yaml`:
...
When using Docker development (`make docker-start`), AstraFlow starts the `provisioner` service only if this provisioner mode is configured. In local or plain Docker sandbox modes, `provisioner` is skipped.

Remote/provisioner backends default to explicit file synchronization because
AstraFlow cannot infer whether their `/mnt/user-data` mount points reference the
...
- Each AstraFlow thread is bound to its E2B sandbox via metadata
...
`allow_host_bash` is intentionally `false` by default. AstraFlow's local sandbox is a host-side convenience mode, not a secure shell isolation boundary. If you need `bash`, prefer `AioSandboxProvider`. Only set `allow_host_bash: true` for fully trusted single-user local workflows.
...
If the configured `host_path` is not visible to the gateway process, AstraFlow logs an error and ignores that mount.
...
When you configure `sandbox.mounts`, AstraFlow exposes those `container_path` values in the agent prompt so the agent can discover and operate on mounted directories directly instead of assuming everything must live under `/mnt/user-data`.

For bare-metal Docker sandbox runs that use localhost, AstraFlow binds the sandbox HTTP port to `127.0.0.1` by default so it is not exposed on every host interface. Docker-outside-of-Docker deployments that connect through `host.docker.internal` keep the broad legacy bind for compatibility. Set `DEER_FLOW_SANDBOX_BIND_HOST` explicitly if your deployment needs a different bind address.
...
`AioSandboxProvider` talks to the sandbox container through the `agent-sandbox` SDK. The Dockerfile for the default `enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest` image is not part of this repository; AstraFlow treats that image as an upstream AIO sandbox runtime.
...
USER root
# Example user dependency; not required by AstraFlow itself.
RUN apt-get update \
...
If you rebuild the runtime from scratch instead of extending the published image, it must expose the same HTTP API used by `agent-sandbox`. AstraFlow currently depends on:

- `sandbox.get_context()`, including `home_dir`
- `shell.exec_command(...)`
- `bash.exec(...)` — only exercised for per-command environment injection (skills that declare `required-secrets`). The `/v1/bash/*` routes exist since upstream all-in-one-sandbox `1.9.3`; on older images (including a `latest` tag still frozen on the `1.0.0.x` line) AstraFlow fails fast with an actionable error instead of surfacing the raw 404. Pin `sandbox.image` to `1.9.3` or newer (e.g. `1.11.0`) and recreate the sandbox container to use `required-secrets` with the AIO sandbox.
- `file.read_file(...)`
...
- `/mnt/user-data` must remain writable because AstraFlow mounts thread workspace, uploads, and outputs there.
- `home_dir` comes from the sandbox context endpoint; do not assume AstraFlow hardcodes it.
- Shell command handling must remain compatible with serialized `exec_command` calls. AstraFlow serializes shell access on the host side to avoid corrupting the sandbox's persistent shell session.
...
2. Restart the AstraFlow service to apply changes

## Environment Variables

AstraFlow supports environment variable substitution using the `$` prefix:
...
AstraFlow searches for configuration in this order:
...
AstraFlow executes agent-generated shell/code through a configurable sandbox
...
matters for AstraFlow because the gateway executes model-generated commands, so a
...
AstraFlow can reuse your Claude Code / Codex CLI subscription login as a model
...
adapters authenticate independently of AstraFlow via their own documented env —
