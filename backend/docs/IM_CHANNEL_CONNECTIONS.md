...
AstraFlow supports user-owned IM channel bindings for Telegram, Slack, Discord, Feishu/Lark, DingTalk, WeChat, and WeCom. The feature reuses the existing `channels.*` runtime configuration, so it works in local and private deployments with the same outbound transports already supported by AstraFlow.
...
A user-owned IM channel connection is a **per-AstraFlow-user bind layer** layered on top of the existing provider bot credentials in `channels.*`. The connection layer adds three things the bot credentials alone cannot give you:

1. **Owner identity** — each `(provider, external account, workspace)` maps to exactly one AstraFlow account (`owner_user_id`). Every run created from that connection runs in the owner's bucket (memory, uploads, outputs, custom agent).
...
channel_connections:
  ...
  # connected AstraFlow user by default. Set this to false only for legacy
...
When `channel_connections.enabled` and `require_bound_identity` are true, auth-enabled deployments reject ordinary unbound IM messages before creating a AstraFlow thread or run. Users must connect the channel from AstraFlow Settings first. Auth-disabled local mode still routes channel messages to the auth-disabled default user, and legacy open-bot behavior can be restored explicitly with `require_bound_identity: false`.

Upgrade note: existing auth-enabled deployments that already have `channel_connections.enabled: true` will start rejecting ordinary unbound IM messages after this field is introduced because `require_bound_identity` defaults to true. Legacy operator-owned/open-bot deployments that intentionally allow unbound platform users to create AstraFlow runs should set `require_bound_identity: false` before upgrading and restart the service.
...
- The existing Telegram long-polling worker receives `/start <code>` and binds that Telegram chat/user to the current AstraFlow user.

Slack:

- The frontend creates a short one-time code.
- The UI shows `Send /connect <code> to the AstraFlow Slack bot.`
- The existing Slack Socket Mode worker receives the message and binds the Slack user/team to the current AstraFlow user.

Discord:

- The frontend creates a short one-time code.
- The UI shows `Send /connect <code> to the AstraFlow Discord bot.`
- The existing Discord Gateway worker receives the message and binds the Discord user/guild to the current AstraFlow user.

Feishu/Lark, DingTalk, WeChat, and WeCom:

- The frontend creates a short one-time code.
- The UI shows `Send /connect <code> to the AstraFlow <Provider> bot.`
- The already-running long-connection or polling worker receives the message and binds the platform user/workspace identity to the current AstraFlow user.
...
- `channel_conversations`: connection-scoped IM conversation to AstraFlow thread mapping.
- `channel_credentials`: reserved for future provider-token flows, not used by the local/private binding flow.

Incoming messages that resolve to a connection carry `connection_id`, `owner_user_id`, and `workspace_id`. `ChannelManager` uses `owner_user_id` as the AstraFlow run user id and preserves the raw platform user id as `channel_user_id`.
...
owner-only file permissions (`0600`); use it only where the AstraFlow data
...
- Stored per-connection credentials use the `channel_credentials` encryption
  path. If stored credential material cannot be decrypted, AstraFlow treats it
