...
AstraFlow supports configurable MCP servers and skills to extend its capabilities, which are loaded from a dedicated `extensions_config.json` file in the project root directory.
...
`tool_call_timeout` only applies to `stdio` servers. `http` and `sse` servers use transport-level timeouts, and AstraFlow logs a warning if `tool_call_timeout` is configured for those transports.

## Filesystem MCP Servers

AstraFlow already provides built-in file tools for thread-scoped workspace access.
Do not add an MCP filesystem server for the same AstraFlow workspace. The
overlapping file tools use different path semantics, which can make LLM tool
selection and file access behavior unstable.

AstraFlow does not currently adapt the MCP Roots mode for filesystem servers. In
particular, it does not publish per-thread MCP roots or map AstraFlow sandbox
paths such as `/mnt/user-data/...` to paths accepted by
`@modelcontextprotocol/server-filesystem`. Use AstraFlow's built-in file tools
for AstraFlow workspace files.

## OAuth Support (HTTP/SSE MCP Servers)

For `http` and `sse` MCP servers, AstraFlow supports OAuth token acquisition and automatic token refresh.
...
material already retained by those systems. Restarting or upgrading AstraFlow does
not rotate credentials or perform historical cleanup; operators must complete
both actions explicitly.

## How It Works

MCP servers expose tools that are automatically discovered and integrated into AstraFlow’s agent system at runtime. Once enabled, these tools become available to agents without additional code changes.
