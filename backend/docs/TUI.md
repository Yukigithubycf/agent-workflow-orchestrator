# AstraFlow Terminal Workbench (TUI)

`deerflow` is a terminal-native workbench for the AstraFlow harness. It runs
...
memory, MCP, and sandbox settings as the rest of AstraFlow.

![AstraFlow TUI](../../docs/tui/tui-preview.svg)
...
semantics as elsewhere in AstraFlow). `/model` and `/threads` open modal pickers.
