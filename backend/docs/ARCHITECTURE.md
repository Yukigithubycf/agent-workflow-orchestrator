...
This document provides a comprehensive overview of the AstraFlow backend architecture.
...
- `threads.py` - `/api/threads/{id}` - Local AstraFlow thread data cleanup after LangGraph deletion
- `artifacts.py` - `/api/threads/{id}/artifacts` - Artifact serving
- `suggestions.py` - `/api/threads/{id}/suggestions` - Follow-up suggestion generation

The web conversation delete flow first deletes Gateway-managed thread state through the LangGraph-compatible route, then the Gateway `threads.py` router removes AstraFlow-managed filesystem data via `Paths.delete_thread_dir()`.
...
3. Gateway removes local AstraFlow-managed files
