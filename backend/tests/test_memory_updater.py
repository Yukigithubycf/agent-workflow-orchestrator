...
def test_apply_updates_skips_same_batch_duplicates_and_keeps_source_metadata() -> None:
    ...
    update_data = {
        "newFacts": [
            ...
            {"content": "User works on AstraFlow", "category": "context", "confidence": 0.87},
        ],
    }
...
def test_clear_memory_data_clears_facts_and_preserves_shared_summaries() -> None:
    ...
    memory["user"]["workContext"]["summary"] = "Working on AstraFlow"
    ...
    assert result["facts"] == []
    assert result["user"]["workContext"]["summary"] == "Working on AstraFlow"
...
def test_import_memory_data_saves_and_returns_imported_memory() -> None:
    imported_memory = _make_memory(
        facts=[
            {
                "id": "fact_import",
                "content": "User works on AstraFlow.",
                "category": "context",
                "confidence": 0.87,
...
class TestUpdateMemoryStructuredResponse:
    ...
    def test_schema_guard_ignores_invalid_update_fields(self):
        ...
        response = '{"user": "bad", "history": [], "newFacts": ["bad", {"content": "User works on AstraFlow", "category": "context", "confidence": 0.91}], "factsToRemove": "bad"}'

        result, storage = self._run_update_with_response(response)

        assert result is True
        assert [fact["content"] for fact in storage.memory["facts"]] == ["User works on AstraFlow"]
