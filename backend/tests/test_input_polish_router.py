...
def test_input_polish_instruction_uses_astraflow_brand_only():
    instruction = input_polish._build_system_instruction()

    assert instruction.startswith("You are AstraFlow's pre-send prompt optimizer.")
    assert ("Deer" + "Flow") not in instruction
    assert "deer-flow" not in instruction.lower()
