...
def test_default_prompt_uses_astraflow_brand_only(monkeypatch):
    config = SimpleNamespace(
        sandbox=SimpleNamespace(mounts=[]),
    ...
    assert "You are AstraFlow, an open-source super agent." in prompt
    assert ("Deer" + "Flow") not in prompt
    assert "deer-flow" not in prompt.lower()
