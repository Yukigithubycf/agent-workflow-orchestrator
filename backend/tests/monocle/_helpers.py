"""Helpers for the AstraFlow Monocle behavioural tests.
...
def run_deerflow(message: str) -> str:
    """Run the AstraFlow agent once and return its response text.

    The model is resolved from ``config.yaml`` (no hardcoded override) so the
    live test exercises AstraFlow's own model-resolution path.
    """
