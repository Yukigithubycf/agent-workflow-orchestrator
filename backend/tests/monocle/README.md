# AstraFlow behavioural tests (Monocle Test Tools)

Trace-based tests for AstraFlow. Monocle records each run as a structured trace
...
- **One offline example** (`test_assertion_api_example`) loads a recorded trace
  ...
  format and the asserter wiring, not AstraFlow's behaviour. Treat it as the
...
it embeds the AstraFlow system prompt as of the recording date and the content
...
they still skip if the AstraFlow app is not importable or `config.yaml` is
...
so there is no hard-coded key requirement. AstraFlow's `web_search` is
...
1. Run AstraFlow under Monocle and capture a trace of a run you are happy with
