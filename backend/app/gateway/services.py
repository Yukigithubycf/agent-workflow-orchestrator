...
async def resolve_trusted_internal_owner_for_attribution(request: Request, owner_user_id: str | None) -> Any | None:
    """Resolve the AstraFlow user used only for trusted internal attribution."""
...
async def start_run(
    ...
    # X-AstraFlow-Owner-User-Id, so they are scoped to that owner instead of
    ...
    try:
        ...
        # Merge AstraFlow-specific context overrides into both ``configurable`` and ``context``.
