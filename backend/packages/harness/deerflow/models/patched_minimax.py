...
``langchain_openai.ChatOpenAI`` currently ignores that field, so AstraFlow's
...
which AstraFlow already understands.
"""
...
class PatchedChatMiniMax(ChatOpenAI):
    ...
    def _strip_user_message_names(payload: dict) -> None:
        ...
        AstraFlow middlewares tag user messages with internal provenance names
