"""Hybrid schema bootstrap for AstraFlow's application tables.
...
| legacy (AstraFlow tables, no alembic)  | ``create_all`` (baseline tables only, as backfill) + ``stamp 0001_baseline`` + ``upgrade head`` |
...
AstraFlow-owned table. ``create_all`` runs first because stamping at
...
def _run_create_all_sync(sync_conn: Any) -> None:
    """Create all AstraFlow-owned tables on *sync_conn*."""
...
async def _sqlite_lock(engine: AsyncEngine):
    ...
    AstraFlow. The 30s ``busy_timeout`` plus idempotent revisions cover the
