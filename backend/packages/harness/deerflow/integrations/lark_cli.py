...
* a **content** SHA-256 over the extracted skill tree, after AstraFlow's shared
...
also manages a writable AstraFlow-owned Gateway CLI under
...
def lark_cli_managed_gateway_dir() -> Path:
    """Gateway-scoped AstraFlow-managed lark-cli install root."""
...
def lark_cli_env_overlay(user_id: str, *, sandbox_paths: bool = False, broker: bool = False) -> dict[str, str]:
    """Environment overlay for lark-cli using AstraFlow-managed credentials.
...
def _ensure_managed_gateway_lark_cli() -> LarkCliProbe:
    """Install/update the AstraFlow-managed Gateway lark-cli.
...
def _content_sha256(root: Path, skill_names: set[str]) -> str:
    ...
    The caller computes this after injecting AstraFlow's shared guidance, so the
...
在 AstraFlow 中，如果 `lark-cli auth status` 或业务命令提示未配置、未登录、token 过期或缺少用户授权：
