"""``TenkiSandbox`` — AstraFlow :class:`Sandbox` backed by a Tenki cloud sandbox.
...
# ``/mnt`` is root-owned, so AstraFlow's ``/mnt/user-data`` virtual prefix is not
...
class TenkiSandbox(Sandbox):
    """AstraFlow Sandbox adapter that delegates to a live Tenki cloud sandbox.

    Args:
        id: AstraFlow-side sandbox id (the provider's cache key).
    ...
    def _resolve_path(self, path: str) -> str:
        """Map AstraFlow virtual paths into the writable sandbox home dir.
    ...
    def execute_command(
        ...
        AstraFlow passes a bash command *string*; it runs through ``sh -lc``.
