...
Runs each AstraFlow sandbox as a [BoxLite](https://github.com/boxlite-ai/boxlite)
...
The `boxlite` package is an optional AstraFlow harness extra, not part of the
...
hosts, such as Windows, should use another sandbox provider or run AstraFlow from
a supported Linux/macOS environment.

**Host requirement:** BoxLite boots micro-VMs, so a Linux host needs KVM — i.e.
nested virtualization when AstraFlow runs inside a cloud VM. macOS uses
Hypervisor.framework. This is the main deployment constraint to weigh vs. the
container-based providers.

## Design

AstraFlow's `Sandbox` contract is synchronous; BoxLite's SDK is async-native and
