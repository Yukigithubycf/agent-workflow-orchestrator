"""``BoxliteBox`` — AstraFlow :class:`Sandbox` backed by a BoxLite micro-VM.
...
``asyncio.to_thread`` worker AstraFlow invokes us from.
...
class BoxliteBox(Sandbox):
    ...
    Args:
        id: AstraFlow-side sandbox id (the BoxLite box id).
    ...
    def _resolve_path(self, path: str) -> str:
        ...
        # so AstraFlow's virtual paths are used as-is; we only reject traversal.
    ...
    def execute_command(
        ...
        AstraFlow passes a bash command *string*; BoxLite's ``exec`` takes argv, so
