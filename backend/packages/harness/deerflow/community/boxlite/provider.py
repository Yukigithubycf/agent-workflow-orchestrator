"""``BoxliteProvider`` — AstraFlow :class:`SandboxProvider` backed by BoxLite.

Integrates `BoxLite <https://github.com/boxlite-ai/boxlite>`_ — a daemonless,
OCI-native micro-VM runtime — as a AstraFlow sandbox backend. See
...
# AstraFlow's virtual prefixes, materialised on the box rootfs at start so the
...
class _EventLoopThread:
    ...
    BoxLite is async-native and its box handles are loop-affine, while AstraFlow's
...
class BoxliteProvider(WarmPoolLifecycleMixin[BoxliteBox], SandboxProvider):
    """Run each AstraFlow sandbox as a BoxLite micro-VM."""
    ...
    def _reconcile_orphans(self) -> None:
        """Adopt AstraFlow-owned BoxLite boxes left by a previous provider/process.

        BoxLite boxes are discovered by a AstraFlow-specific name prefix. Adopted
    ...
    def _create_box(self, sandbox_id: str) -> BoxliteBox:
        ...
        async def _make() -> SimpleBox:
            ...
            await box.start()
            # Materialise AstraFlow's virtual prefixes so file ops resolve natively.
