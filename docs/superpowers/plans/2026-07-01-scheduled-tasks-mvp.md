...
**Goal:** Build a first-class scheduled-task MVP for AstraFlow with durable backend scheduling, a workspace management page, run history, and real-path validation, limited to thread-attached agent runs with `once` and `cron` schedules.
...
- Scheduled executions must reuse the normal AstraFlow run lifecycle rather than introducing a parallel agent execution path.
...
- observe task trigger and resulting AstraFlow run
