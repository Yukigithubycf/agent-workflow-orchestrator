# AstraFlow TUI - Product and Engineering Spec

**Date**: 2026-06-13
**Status**: Draft, ready for RFC discussion
**Scope**: Hermes-like terminal UI for the AstraFlow harness
...
AstraFlow needs the same product shape:
...
The AstraFlow TUI should borrow product ideas, not implementation details, from Hermes:
...
1. As a AstraFlow user, I want `deerflow` to open an interactive terminal UI, so that I can work without a browser.
2. As a AstraFlow user, I want to continue the most recent thread, so that I can resume work quickly.
3. As a AstraFlow user, I want to resume a specific thread by id or title, so that I can return to older work.
4. As a AstraFlow user, I want a fixed composer with multiline editing, so that long prompts are comfortable to write.
5. As a AstraFlow user, I want slash-command autocomplete, so that skills and commands are discoverable.
6. As a AstraFlow user, I want `/model` to open a model picker, so that model switching is not a fragile text-only workflow.
7. As a AstraFlow user, I want `/threads` or `/switch` to open a live thread switcher, so that I can move between active conversations.
8. As a AstraFlow user, I want `/skills` to browse enabled and available skills, so that I can understand what the agent can do.
9. As a AstraFlow user, I want tool calls and results to stream into a collapsible activity panel, so that I can inspect execution without losing the main answer.
10. As a AstraFlow user, I want uploads and generated artifacts visible in a side panel, so that file-heavy workflows stay manageable.
11. As a AstraFlow user, I want memory status and relevant injected facts visible on demand, so that persistent context is not mysterious.
12. As a AstraFlow user, I want MCP servers and built-in tools visible on demand, so that tool availability is clear.
13. As a AstraFlow user, I want status-line indicators for model, thread, token usage, run state, and sandbox mode, so that I know what environment I am driving.
14. As a AstraFlow user, I want to interrupt a running agent with `Ctrl+C` or a new message, so that I can redirect work without restarting the TUI.
15. As a AstraFlow user, I want clarification prompts to appear as focused overlays, so that I can answer them quickly.
16. As a AstraFlow user, I want file path paste handling, so that pasted paths can become uploads or prompt text intentionally.
17. As a AstraFlow user, I want command history and prompt drafts to survive transient UI redraws, so that I do not lose work.
...
Slash commands should be TUI-owned affordances over existing AstraFlow capabilities:
...
5. The first TUI should prefer a Python-native TUI stack unless a prototype proves a Node/React terminal stack is materially better for AstraFlow. Python-native keeps packaging and runtime closer to the harness.
...
3. Remote HTTP transport to an already-running AstraFlow server.
4. Full desktop app.
5. Voice mode.
...
The core product decision is TUI-first, CLI-second. `deerflow` should feel like a terminal workbench for the AstraFlow harness. Headless commands still matter, but they support automation rather than define the interactive product.
