# AstraFlow Scheduled Tasks MVP Design

**Date**: 2026-07-01
**Status**: Approved for implementation
**Scope**: First-class scheduled-task management for AstraFlow web workspace

---

## Problem Statement

AstraFlow main does not ship a real scheduled-task product surface today. The repository already has internal timers, worker pools, and run persistence, but users cannot create, inspect, pause, resume, trigger, or delete durable background tasks from the product.

This creates three concrete problems:

1. Users cannot automate recurring AstraFlow work such as daily summaries, periodic follow-ups, or recurring repo triage from the normal workspace.
...
2. **Shared execution path**: scheduled executions must launch through the existing AstraFlow run lifecycle, not a parallel agent path.
...
This MVP combines the right parts of prior AstraFlow cron attempts without inheriting their problems:
...
This keeps schedule identity separate from AstraFlow `runs`, which already model one concrete execution attempt.

## User Stories

1. As a AstraFlow user, I want to create a one-time task that can run in a fresh thread, so periodic automation does not silently accumulate old context.
2. As a AstraFlow user, I want to create a recurring cron task that can either reuse a thread or create a fresh thread per run, so I can choose between continuity and isolation explicitly.
3. As a AstraFlow user, I want to see next run time, last run result, and last error at a glance, so I know whether automation is healthy.
4. As a AstraFlow user, I want to pause and resume a task, so I can stop automation without deleting configuration.
5. As a AstraFlow user, I want to trigger a task manually, so I can test or re-run it on demand.
6. As a AstraFlow user, I want to inspect task run history, so I can audit what happened.
7. As a AstraFlow user, I want tasks to be owner-scoped, so no other user can list or mutate my automations.
...
When it fires, AstraFlow launches a normal run, but the execution thread is selected by `context_mode`.
...
1. `fresh_thread_per_run` — default. Each scheduled occurrence creates a fresh AstraFlow thread.
...
- `last_run_id`: nullable AstraFlow run id
- `last_thread_id`: nullable AstraFlow thread id from the latest execution
...
- `run_id`: nullable AstraFlow run id
...
This run ledger is distinct from AstraFlow `runs` because:

1. a scheduled occurrence may fail before a AstraFlow run is created,
...
2. for each claimed task:
   ...
   - dispatch a normal AstraFlow run through shared run-launch helper,
...
4. manual trigger creates a normal AstraFlow run through shared launch helper
...
4. confirm linked AstraFlow run exists
