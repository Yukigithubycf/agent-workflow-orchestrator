...
AstraFlow can use a remote OpenViking server as an optional long-term memory
...
- Hard isolation by hashing each AstraFlow `(user_id, agent_name)` scope into a
  separate OpenViking trusted user identity.
- Local bounded message watermarks under AstraFlow's runtime home. An ordered
...
AstraFlow passes trusted `X-OpenViking-Account` and
...
For a locally installed AstraFlow process, use
`http://127.0.0.1:1933`. For a AstraFlow container connecting to OpenViking on
...
Create the standard AstraFlow local files if they no longer exist:
...
Set at least the normal AstraFlow secrets in `.env`, including
...
Then start AstraFlow with the same overlay:
...
Open AstraFlow at <http://localhost:2026> and OpenViking Studio at
...
Set `base_url: http://127.0.0.1:1933`, then start AstraFlow normally:
...
The AstraFlow entrypoint is <http://localhost:2026>.

## Failure behavior

- Invalid backend configuration fails loudly; AstraFlow never silently writes
  to DeerMem instead.
...
- Once a message batch is accepted, AstraFlow persists a submitted-message
