...
> 状态：设计稿
> 范围：通过 HTTP 将 OpenViking 接入为 AstraFlow 的可选 `MemoryManager` 后端
> 默认行为：DeerMem 继续作为默认后端，OpenViking 需要显式启用
> 非目标：本文不包含代码实现、DeerMem 存量数据迁移、OpenViking 服务端开发

## 1. 背景

AstraFlow 已提供可插拔的 `MemoryManager` 接口。内置的 DeerMem 负责从对话中提取事实、持久化事实并向后续对话注入记忆；`noop` 则提供最小空实现。后端发现器会扫描：
...
因此，本方案不在 AstraFlow 内重复实现 OpenViking 已具备的抽取、去重、合并和向量索引逻辑，而是在 AstraFlow 的 `MemoryManager` 边界增加一个 HTTP 适配器。

## 2. 目标

首版实现以下闭环：

1. AstraFlow 能通过配置选择 `openviking` 记忆后端。
2. 每轮对话完成后，AstraFlow 将本轮有效消息提交到对应的 OpenViking Session。
3. OpenViking 完成归档及异步长期记忆提取。
4. 新一轮对话开始时，AstraFlow 按当前用户、Agent 和线程范围检索相关记忆。
5. 检索结果被格式化为有长度上限的纯文本，通过现有 prompt 注入路径提供给 Agent。
6. OpenViking 短暂不可用时，AstraFlow 主对话链路可以按配置降级，并留下可观测错误。
7. 不同 AstraFlow 用户之间保持强隔离；不同 Agent 的专属记忆不会互相污染。

## 3. 非目标

以下内容不进入首版：

- 不替换 AstraFlow 的 thread、checkpoint、run event 等持久化系统。
- 不把 OpenViking 嵌入 AstraFlow Gateway 进程。
- 不修改 OpenViking 服务端。
- 不迁移现有 DeerMem Markdown/JSON 数据。
- 不接管 AstraFlow 的资源库、上传文件或 Skill 存储。
- 不保证现有 Memory 页面上的 fact 新增、编辑、删除按钮可用于 OpenViking。
- 不将 OpenViking 原生多类型记忆强行压平成 DeerMem 的事实模型。
- 不在首版启用 `memory.mode: tool`。
- 不在 AstraFlow 内再次调用 LLM 进行事实抽取、去重或合并。
...
OpenViking 作为独立服务运行。AstraFlow 只依赖其 HTTP API，不依赖 OpenViking 的 Python 嵌入式运行时。

选择 HTTP 模式的原因：

- 避免把 OpenViking 的模型、向量库、Rust/C++ 扩展和后台任务依赖带入 Gateway。
- OpenViking 可独立升级、扩容、监控和持久化。
- 一个 OpenViking 服务可以服务多个 AstraFlow Gateway 实例。
- HTTP 边界更容易进行超时、熔断、认证和契约测试。
- 降低 AstraFlow 与 OpenViking Python SDK 版本的耦合。
...
定义并验证 OpenViking 私有配置，禁止读取 AstraFlow 全局配置单例。所有值均从 `backend_config` 或明确的环境变量引用取得。
...
这些模型不暴露到 AstraFlow 公共 API，也不导入 OpenViking SDK 类型。
...
## 6. AstraFlow 与 OpenViking 的概念映射

### 6.1 租户与用户

推荐映射：

| AstraFlow | OpenViking | 说明 |
|---|---|---|
| 一个 AstraFlow 部署或业务工作区 | `account` | 外层租户边界 |
...
OpenViking 的 `user` 是强数据边界。每个请求必须显式带上当前 AstraFlow 用户身份，不能依赖 OpenViking 的 `default/default` 开发身份。

首版推荐使用 OpenViking `trusted` 认证模式，由 AstraFlow Gateway 向内部 OpenViking 服务传递：

```text
X-OpenViking-Account: <configured account>
X-OpenViking-User: <current AstraFlow user_id>
...
- OpenViking 仅暴露在可信内网；
- 外部客户端不能绕过 AstraFlow 直接伪造 account/user header；
- AstraFlow 不接受客户端传入的 OpenViking account/user 值；
- user 值只来自 AstraFlow 已认证的 runtime user context；
...
`canonicalize` 必须与 AstraFlow Agent 命名规则一致：
...
这样不依赖 OpenViking 的实验性 Agent tag/URI 过滤语义，不同 AstraFlow
Agent 不可能通过一次普通用户级检索读到彼此的记忆。代价是同一 AstraFlow
...
消息过滤应由适配器自己的纯函数完成并单独测试。后端可以消费 AstraFlow 工厂传入的 `should_keep_hidden_message` hook，但不能直接导入 AstraFlow 的消息过滤内部模块。
...
- 若 OpenViking API 支持客户端 message ID，应始终发送 AstraFlow 的稳定 message ID。
...
如果 AstraFlow 消息缺少稳定 ID，应基于角色、规范化内容和在本轮中的稳定位置生成适配器 ID，但该方案只作为兼容路径。
...
- `add_nowait`：语义为“不要因 AstraFlow 总结丢失消息”，仍应立即完成上传和 commit 接受确认；它不是 fire-and-forget。
...
返回值转换为 AstraFlow `memory_search` 可消费的字典：
...
`get_context` 返回纯文本，由 AstraFlow 现有调用点包裹进 `<memory>`。适配器自身不得再添加 `<memory>` 标签。
...
由于 `supports_search=True`，技术上可通过 `MemoryManager` 的 tool-mode invariant，但首版配置校验应明确拒绝 `mode: tool`。原因是 AstraFlow tool 模式不仅需要 `search`，还暴露 `memory_add/update/delete`，而首版不实现 fact CRUD。
...
优先复用 AstraFlow 已声明的 HTTP 客户端库。若使用 `httpx`：
...
如果部署允许多个 Gateway 副本同时处理同一 thread，本地水位文件不足以提供全局一致性。此时必须依赖 OpenViking 稳定 message ID/幂等键，或把水位迁移到 AstraFlow 的共享数据库。首版发布前必须明确支持的部署拓扑。
...
`MemoryCallbacks.on_memory_llm_call` 不适用于远程 OpenViking 内部的 LLM 调用，因为 AstraFlow 看不到该 LLM 边界。AstraFlow 应记录 HTTP 调用 span，并由 OpenViking 自己提供服务端模型调用可观测性。
...
后续可增加可选 Compose profile，而不把 OpenViking 变成 AstraFlow 的强制服务。
...
退出条件：mock 环境跑通 AstraFlow 写入与检索注入闭环。
...
-后续对话能召回已完成提取的长期记忆；
-注入文本有明确长度上限；
-搜索结果可被 AstraFlow `memory_search` 兼容消费；
- OpenViking 内部完成抽取，AstraFlow 不执行第二套抽取。

隔离：

-不同 AstraFlow user 无法互相读写 Session 或记忆；
...
- OpenViking backend 目录除 `MemoryManager` 契约外不导入 AstraFlow 内部模块；
-现有 DeerMem/noop 测试全部通过。

## 19. 回滚

回滚只修改配置并重启 AstraFlow：
...
2. AstraFlow 的目标部署是否允许多个 Gateway 副本并发处理同一 thread？
