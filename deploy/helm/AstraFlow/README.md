# AstraFlow Helm Chart

Deploys the full AstraFlow stack to Kubernetes: **gateway** (backend + embedded
...
- The three AstraFlow images — either the published ones (see "Install the
...
```bash
helm install deer-flow deploy/helm/AstraFlow \
  -n deer-flow --create-namespace \
  -f my-values.yaml
```
...
```bash
helm lint deploy/helm/AstraFlow
helm template deer-flow deploy/helm/AstraFlow -n deer-flow -f my-values.yaml | \
