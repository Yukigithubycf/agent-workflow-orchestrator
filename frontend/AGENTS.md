...
This file provides guidance to AI coding agents (Claude Code, Codex, and others) when working with the AstraFlow frontend. It is the source of truth; the sibling `CLAUDE.md` imports it via `@AGENTS.md`.

## Project Overview

AstraFlow Frontend is a Next.js 16 web interface for an AI agent system. It communicates with a LangGraph-based backend to provide thread-based AI conversations with streaming responses, artifacts, and a skills/tools system.
...
The customized workspace currently hides navigation for custom-agent configuration
and scheduled tasks. Settings also exposes only Appearance, Memory, Tools, and Skills;
Account, Notification, Channels, Integrations, and About remain implemented but are
intentionally absent from both the settings navigation and `?settings=` deep links.
Per-thread export and share controls are also intentionally hidden from chat headers
and recent-chat menus; their underlying export helpers remain implemented.
The sidebar footer exposes Settings as a direct button that opens the shared dialog on
Appearance; it does not use an intermediate "Settings and more" dropdown.
...
3. `useThreadHistory` loads persisted conversation pages from `GET /api/threads/{id}/messages/page`, preserving the backend's thread-global event `seq`; rendering overlays checkpoint/live copies at their matching canonical identities (a summarized checkpoint may contain a protected early input plus a recent tail). Dynamic-context injection moves the real user input from message ID `X` to `X__user` while a hidden reminder takes `X`; the frontend normalizes those IDs to one display identity so stream/history reconciliation cannot render the original prompt twice, and completed runs discard any remaining optimistic input copy. Context-compaction rescue diffs every retained visible identity rather than slicing at the first anchor, and keeps a run-scoped ledger of committed visible messages so replacement updates and repeated rolling checkpoint windows cannot erase an already displayed step. The resolver suppresses checkpoint/transient prefixes whose canonical position is still behind an unloaded cursor page instead of collapsing that unknown gap before a recent anchor, then adds optimistic messages without timestamp re-sorting. While a compaction bridge is active, the final history/bridge/live merge reuses the committed ledger order for identities already shown, keeping a retained dynamic-context `X__user` input ahead of rescued processing steps when run-event history is stale and no canonical anchor exists; identities first seen after that frame keep their current source slots. History invalidation preserves already-loaded pages so their established ordering positions are not discarded.
...
The standard chat header owns a right-side task-examples drawer. Its first-visit
coachmark is recorded in guarded `localStorage` only after the user opens the
drawer or explicitly dismisses the coachmark, so a transient mount cannot
silently consume the onboarding. Choosing an example replaces the current
composer draft, clears any one-turn slash-skill selection, and sets the
example's recommended `thinking`, `pro`, or `ultra` mode without submitting the
request. `InputBox.prefillRequest` is a one-shot id/text command so the same
example can be selected repeatedly without remounting the composer or losing
attachments.
...
- **Subtask galaxy, step history, and runtime metadata** (`core/tasks/`) — `components/workspace/messages/subtask-galaxy.tsx` renders each delegated batch as a lead-agent core with stable orbit nodes. `core/tasks/galaxy.ts` owns the pure phase/count and one-/two-ring layout model; keep visual components out of that module, and preserve enough vertical radius for task cards to clear the lead core. Active desktop nodes float subtly while SVG dash offsets make orbit and connection strokes flow; every continuous motion path must honor `prefers-reduced-motion`. Only a live `in_progress -> completed` transition emits a result-arrival animation, so hydrating historical terminal tasks never replays convergence. Failed tasks remain visibly failed and do not increment the merged-result count. Selecting a node opens a right-side `Sheet` containing the existing `SubtaskCard` detail timeline with `defaultOpen`, preserving prompt, reasoning/tool steps, result, error, model, token usage, and historical event backfill without expanding the transcript vertically. The mobile layout uses the same node/drawer state without the spatial orbit. `Subtask.steps[]` is accumulated live from `task_running` events (appended via `mergeSteps`, not overwritten) and backfilled on expand for historical runs by `fetchSubtaskSteps`, which pages the events endpoint scoped to one task (GET `/runs/{runId}/events?event_types=subagent.step&task_id=…&after_seq=…`) until a short page, so the run-wide limit can't truncate the timeline. `task_started` carries the effective `model_name`; `task_running` carries a cumulative usage snapshot after each completed LLM call. `core/tasks/lifecycle.ts` normalizes these additive events, and `computeNextSubtask` keeps the largest cumulative total so replayed or late SSE frames cannot double-count or roll the folded card backward. Terminal ToolMessage metadata (`subagent_model_name` / `subagent_token_usage`) restores the same values from normal history after reload; no per-card event fetch is needed. `core/tasks/steps.ts` is the pure step model: `messageToStep` (live), `eventsToSteps` (reload), `mergeSteps` (dedup by `message_index`), and `stepsForDisplay` (what the card renders — keeps tool steps + AI steps with text, drops the trailing final-answer AI step when completed since it's shown as `result`). `core/tasks/context.tsx`'s `useUpdateSubtask` applies updates against a `tasksRef` mirroring the latest state (not a closure snapshot), so a late-resolving `fetchSubtaskSteps` backfill merges into current state instead of clobbering SSE steps or sibling subtasks that arrived meanwhile. The owning `run_id` is carried onto history content messages in `buildVisibleHistoryMessages` so the detail can resolve the events endpoint.
...
- The same `ResizablePanelGroup` must receive a complete `defaultLayout` for
  both stable panel ids. Individual `Panel.defaultSize` values are applied only
  after measurement; without the group layout, server rendering gives both
  panels `flex-grow: 1` and flashes an empty half-width right panel before
  hydration.
- Main and sidecar composers rely on the generated `InputGroup` for their
  single visible border. Do not add a second border to the outer `PromptInput`
  form: it changes the main composer height by two pixels and breaks the shared
  bottom-aligned layout contract.
