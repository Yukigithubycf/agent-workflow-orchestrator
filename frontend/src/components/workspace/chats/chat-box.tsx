...
const RIGHT_PANEL_DEFAULT_PERCENTAGE = 40;
const RIGHT_PANEL_DEFAULT_SIZE = `${RIGHT_PANEL_DEFAULT_PERCENTAGE}%`;
...
const ChatBox: React.FC<{
  ...
  // Read once: the effect below owns every later open/close. Group-level
  // `defaultLayout` is also required because Panel defaults are measured only
  // after mount; without it the server-rendered first frame gives both panels
  // flex-grow: 1 and briefly paints an empty half-width panel.
  const [initialRightPanelPercentage] = useState(() =>
    rightPanelOpen ? RIGHT_PANEL_DEFAULT_PERCENTAGE : 0,
  );
  const initialRightPanelSize = `${initialRightPanelPercentage}%`;
  const initialPanelLayout = useMemo<Layout>(
    () => ({
      [`${resizableIdBase}-chat`]: 100 - initialRightPanelPercentage,
      [`${resizableIdBase}-side`]: initialRightPanelPercentage,
    }),
    [initialRightPanelPercentage, resizableIdBase],
  );
