...
export function InputBox({
  ...
  initialValue?: string;
  /**
   * One-shot external draft replacement, used by task examples. The id must
   * change for each request so selecting the same example twice still works.
   */
  ...
  const lastPrefillRequestIdRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    ...
    promptHistoryIndexRef.current = null;
    promptHistoryDraftRef.current = "";
    setTextInput(prefillRequest.text);
  ...
  return (
    ...
    >
      ...
      <PromptInput
        className={cn(
          "bg-card/92 relative z-10 rounded-md backdrop-blur-sm transition-[border-color,box-shadow] duration-200 ease-out focus-within:shadow-[0_0_0_3px_color-mix(in_oklch,var(--primary)_12%,transparent)] *:data-[slot='input-group']:rounded-md",
          polishingInput &&
...
function SuggestionList({
  ...
  return (
    <Suggestions className="grid min-h-16 w-full max-w-full grid-cols-2 justify-center gap-2 px-4 sm:w-auto sm:grid-cols-3 sm:px-0">
      <ConfettiButton
        className="h-9 w-full cursor-pointer rounded-md border-amber-500/30 bg-amber-500/8 px-3 text-xs font-medium text-amber-700 hover:bg-amber-500/15 dark:text-amber-300"
        variant="outline"
        size="sm"
      ...
      {t.inputBox.suggestions.map((suggestion) => (
        <Suggestion
          ...
          className="bg-card/70 hover:border-primary/35 hover:text-foreground h-9 w-full rounded-md px-3 font-medium shadow-sm"
