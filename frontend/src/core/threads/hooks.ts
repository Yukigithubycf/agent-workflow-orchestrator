...
function canonicalMessageId(message: Message): string | undefined {
  ...
  if (
    ...
    const originalId = message.id.slice(
      0,
      -INJECTED_USER_MESSAGE_ID_SUFFIX.length,
    );
    return originalId || message.id;
  }
  return message.id;
}
...
function messageIdentity(message: Message): string | undefined {
  ...
  const messageId = canonicalMessageId(message);
  if (messageId) {
...
/**
 * Keep identities that were already committed to the UI in their established
 * relative order while allowing fresh message objects to replace stale copies.
 */
export function stabilizeMessagesByEstablishedOrder(
  messages: Message[],
  establishedOrder: readonly string[],
): Message[] {
  if (messages.length < 2 || establishedOrder.length < 2) {
    return messages;
  }

  const establishedIndex = new Map(
    establishedOrder.map((identity, index) => [identity, index]),
  );
  const establishedMessages = messages
    ...
    .filter(
      (entry): entry is typeof entry & { identity: string } =>
        entry.identity !== undefined && establishedIndex.has(entry.identity),
    );
  if (establishedMessages.length < 2) {
    return messages;
  }

  establishedMessages.sort((left, right) => {
    const orderDelta =
      establishedIndex.get(left.identity)! -
      establishedIndex.get(right.identity)!;
    return orderDelta || left.currentIndex - right.currentIndex;
  });

  let establishedCursor = 0;
  let changed = false;
  const stabilized = messages.map((message) => {
    const identity = messageIdentity(message);
    if (!identity || !establishedIndex.has(identity)) {
      return message;
    }
    const replacement = establishedMessages[establishedCursor++]!.message;
    changed ||= replacement !== message;
    return replacement;
  });
...
export function useThreadStream({
  ...
  const thread = useStream<AgentThreadState>({
    ...
    onFinish(state) {
      // A completed run has accepted its input. Drop any remaining local
      // optimistic copy even if stream ordering or a first-turn ID swap kept
      // the normal human-count/identity confirmation effects from firing.
      setOptimisticMessages([]);
      setOptimisticThreadId(null);
  ...
  const hasActiveTransientHistoryBridge =
    transientHistoryBridgeRef.current.length > 0 &&
    transientHistoryThreadIdRef.current === threadId;

  const transientHistoryOrder = hasActiveTransientHistoryBridge
  ...
  const mergedMessages = useMemo(() => {
    ...
    return hasActiveTransientHistoryBridge
      ? stabilizeMessagesByEstablishedOrder(merged, previouslyRenderedOrder)
      : merged;
  }, [
    hasActiveTransientHistoryBridge,
    previouslyRenderedOrder,
    renderMessages,
    threadId,
    transientHistoryOrder,
