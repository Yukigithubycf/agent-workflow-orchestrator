...
export interface Translations {
  ...
  taskExamples: {
    ...
    exampleLabel: string;
    useExample: string;
    worldModelsPrompt: string;
    aiNewsPrompt: string;
    codingAgentsPrompt: string;
  };
  ...
  subtasks: {
    ...
    galaxy: {
      leadAgent: string;
      dispatching: string;
      parallel: (count: number) => string;
      converging: string;
      completed: string;
      mergedProgress: (merged: number, total: number, failed: number) => string;
      selectTask: string;
    };
  };
