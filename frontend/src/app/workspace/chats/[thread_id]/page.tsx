...
export default function ChatPage() {
  ...
  const taskExampleRequestIdRef = useRef(0);
  const [taskExamplePrefill, setTaskExamplePrefill] = useState<{
    id: number;
    text: string;
  } | null>(null);
  ...
  const handleTaskExampleSelect = useCallback(
    (example: TaskExample) => {
      setSettings("context", {
        mode: example.mode,
        reasoning_effort:
      ...
      taskExampleRequestIdRef.current += 1;
      setTaskExamplePrefill({
        id: taskExampleRequestIdRef.current,
        text: example.prompt,
      });
    },
    [setSettings],
    ...
    <ThreadContext.Provider value={{ thread, isMock }}>
      ...
      >
        <ChatBox threadId={threadId} browserEnabled={browserEnabled}>
          <div className="astraflow-workbench relative flex size-full min-h-0 justify-between">
            <header
              className={cn(
                // The welcome composer is a full-height absolute layer at
                // z-30. Keep the header above it so its controls remain
                // clickable before the first message is sent.
            ...
            >
              ...
              <div className="flex shrink-0 items-center gap-2">
                <TaskExamplesDrawer
                  ...
                  onSelect={handleTaskExampleSelect}
                />
                <TokenUsageIndicator
                  threadId={isNewThread ? undefined : threadId}
            ...
            <main className="flex min-h-0 max-w-full grow flex-col">
              ...
              <div
                className={cn(
                  ...
                  isWelcomeMode
                    ? "pointer-events-none absolute top-0 items-center"
              ...
              >
                ...
                >
                  ...
                  {mountedRef.current ? (
                    <InputBox
                      className="border-primary/25 bg-card/95 dark:bg-card/90 w-full shadow-[0_18px_60px_-28px_color-mix(in_oklch,var(--primary)_45%,transparent)]"
                      ...
                      context={settings.context}
                      prefillRequest={taskExamplePrefill}
                      disabled={
                        isMock ||
                        env.NEXT_PUBLIC_STATIC_WEBSITE_ONLY === "true" ||
                        isUploading ||
