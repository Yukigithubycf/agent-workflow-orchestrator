---
title: 配置
description: 本页面涵盖 AstraFlow 应用的所有配置层——`config.yaml`、前端环境变量、`extensions_config.json` 和运行时环境变量。
---
...
本页面涵盖 AstraFlow 应用的所有配置层——`config.yaml`、前端环境变量、`extensions_config.json` 和运行时环境变量。

## config.yaml

`config.yaml` 是 AstraFlow 的主要配置文件。所有 Agent 行为、模型选择、工具加载和运行时功能都由它控制。
...
AstraFlow 使用 `database` 配置同时管理 LangGraph 检查点数据和应用数据，例如运行记录、反馈和线程元数据。
...
<Callout type="warning">
  不要为 AstraFlow 工作区文件引入 MCP filesystem server。它会与 AstraFlow
  内置文件工具形成路径语义不同的重复能力，让 LLM 行为不稳定。AstraFlow
...
技能启用状态会反映在 `extensions_config.json` 中。你可以直接编辑它，或通过 AstraFlow 应用界面进行管理。

## 运行时环境变量

这些变量在 AstraFlow 进程中设置（通过 `.env`、Docker 环境或 shell）：
