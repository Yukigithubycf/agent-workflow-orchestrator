---
title: Quick Start
description: This guide walks you through starting AstraFlow App on your local machine using the `make dev` workflow. Gateway, Frontend, and nginx start together and are accessible through a single URL.
---
...
<Callout type="info" emoji="⚡">
  Get AstraFlow App running locally in about 10 minutes. You need a machine with
  Python 3.12+, Node.js 22+, and at least one LLM API key.
</Callout>

This guide walks you through starting AstraFlow App on your local machine using the `make dev` workflow. Gateway, Frontend, and nginx start together and are accessible through a single URL.
...
```bash
git clone https://github.com/Yukigithubycf/AstraFlow.git
cd deer-flow
```
