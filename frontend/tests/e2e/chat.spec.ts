...
test.describe("Chat workspace", () => {
  ...
  test("server-renders a closed right panel without a blank split", async ({
    browser,
  }, testInfo) => {
    ...
    const groupBox = await page
      .locator('[data-group][id="workspace-chats-new-group"]')
      .boundingBox();
    const chatBox = await page
      .locator('[data-panel][id="workspace-chats-new-chat"]')
      .boundingBox();
    const sideBox = await page
      .locator('[data-panel][id="workspace-chats-new-side"]')
      .boundingBox();

    expect(groupBox).not.toBeNull();
    expect(chatBox).not.toBeNull();
    expect(sideBox).not.toBeNull();
    expect(chatBox!.width / groupBox!.width).toBeGreaterThan(0.98);
    expect(sideBox!.width).toBeLessThan(1);

    await context.close();
  });

  test("fills the composer and switches mode from task examples", async ({
    page,
  }) => {
    await page.goto("/workspace/chats/new");
    ...
    const textarea = page.getByPlaceholder(/how can i assist you/i);
    await expect(textarea).toBeVisible({ timeout: 15_000 });
    await expect(page.getByTestId("task-examples-onboarding")).toBeVisible();

    // The welcome composer covers the full viewport. Its transparent area
    // must not intercept clicks intended for the header trigger.
    await page.getByTestId("task-examples-trigger").click();
    await expect(
      page.getByRole("heading", { name: "Task examples" }),
    ).toBeVisible();

    await page.getByTestId("task-example-world-models").click();
    await expect(textarea).toHaveValue("Research world models.");
    await expect(
      page.getByText("Reasoning", { exact: true }).first(),
    ).toBeVisible();

    await page.getByTestId("task-examples-trigger").click();
    await page.getByTestId("task-example-ai-news").click();
    await expect(textarea).toHaveValue(
      "Summarize five frontier AI news stories from the past week.",
    );
    await expect(page.getByText("Pro", { exact: true }).first()).toBeVisible();

    await page.getByTestId("task-examples-trigger").click();
    await page.getByTestId("task-example-coding-agents").click();
    await expect(textarea).toHaveValue(
    ...
    await expect(
      page.getByText("Ultra", { exact: true }).first(),
    ).toBeVisible();
  });
  ...
  test("can type a message in the input box", async ({ page }) => {
    ...
    await textarea.fill("Hello, AstraFlow!");
    await expect(textarea).toHaveValue("Hello, AstraFlow!");
  });
  ...
  test("restores a draft after reload and clears it after sending", async ({
    ...
    await expect(page.getByText("Hello from AstraFlow!")).toBeVisible({
      timeout: 10_000,
    });
