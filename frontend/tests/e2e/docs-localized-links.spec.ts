...
test.describe("Localized documentation links", () => {
  test("keeps English card navigation in the English docs", async ({
    ...
    const card = page.locator("a.nextra-card", { hasText: "Why AstraFlow" });
    ...
    await expect(page.locator("main h1")).toContainText("Why AstraFlow");
  });
  ...
  test("uses valid repository links for documentation feedback and edits", async ({
    ...
    await expect(
      ...
      "href",
      /github\.com\/Yukigithubycf\/AstraFlow\/issues\/new/,
    );
    await expect(
      page.getByRole("link", { name: "Edit this page" }),
    ).toHaveAttribute(
      "href",
      "https://github.com/Yukigithubycf/AstraFlow/tree/main/frontend/src/content/en/application/quick-start.mdx",
    );
  });
});
