...
test.describe("Thread history", () => {
  ...
  test("thread export and share controls are hidden", async ({ page }) => {
    mockLangGraphAPI(page, { threads: THREADS });
    ...
    await threadItem.getByRole("button", { name: /more/i }).click();

    await expect(page.getByRole("menuitem", { name: /^share$/i })).toHaveCount(
      0,
    );
    await expect(page.getByRole("menuitem", { name: /^export$/i })).toHaveCount(
      0,
    );
  });
  ...
  test("new chat resets immediately after a history-only thread URL update", async ({
    ...
    await expect(page.getByText("Hello from AstraFlow!")).toHaveCount(0);
    await expect(textarea).toBeVisible();
  });
