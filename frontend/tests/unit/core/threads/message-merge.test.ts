...
import {
  ...
  stabilizeMessagesByEstablishedOrder,
...
test("mergeMessages treats a dynamic-context __user copy as the original human message", () => {
  const originalHuman = {
    id: "human-1",
    type: "human",
    content: "compare three coding agents",
  } as Message;
  const hiddenReminder = {
    id: "human-1",
    type: "system",
  ...
  expect(
    mergeMessages([originalHuman], [hiddenReminder, injectedUser], []),
  ).toEqual([injectedUser]);
});

test("getSummarizationMiddlewareMessages matches AstraFlow summarization update keys", () => {
  const removeAll = {
    id: "__remove_all__",
    type: "remove",
    content: "",
...
test("areOptimisticMessagesConfirmed accepts a dynamic-context __user id alias", () => {
  const optimisticHuman = {
    id: "replacement-human-1",
    type: "human",
    content: "edited question",
  ...
  const serverHuman = {
    id: "replacement-human-1__user",
    type: "human",
    content: "edited question",
  } as Message;

  expect(areOptimisticMessagesConfirmed([optimisticHuman], [serverHuman])).toBe(
    true,
  );
});
...
test("stabilizes a retained dynamic-context user before rescued processing steps without a history anchor", () => {
  const originalUser = {
    id: "current-user",
    type: "human",
    content: "Research world models",
  } as Message;
  const hiddenReminder = {
    id: "current-user",
    type: "system",
    content: "hidden dynamic context",
    additional_kwargs: {
      hide_from_ui: true,
      dynamic_context_reminder: true,
    },
  ...
  const injectedUser = {
    id: "current-user__user",
    type: "human",
    content: "Research world models",
  } as Message;
  const steps = Array.from({ length: 4 }, (_, index) => ({
    id: `research-step-${index + 1}`,
    type: "ai",
    content: `step ${index + 1}`,
  })) as Message[];
  const previouslyRendered = [originalUser, ...steps];
  const postCompactionWindow = [
    hiddenReminder,
    injectedUser,
    ...steps.slice(2),
  ];
  ...
  const bridgeOrder = mergeTransientHistoryBridgeOrder([], moved);
  const previouslyRenderedOrder = [
    "message:current-user",
    ...steps.map((message) => `message:${message.id}`),
  ];
  const effectiveHistory = resolveTransientHistoryBridge(
    [],
    moved,
  ...
  const transientlyMisordered = mergeMessages(
    effectiveHistory,
    postCompactionWindow,
    [],
  );

  expect(transientlyMisordered.map((message) => message.id)).toEqual([
    "research-step-1",
    "research-step-2",
    "current-user__user",
    "research-step-3",
...
test("stabilizeMessagesByEstablishedOrder leaves new message slots in current source order", () => {
  const olderHistory = {
    id: "older-history",
    type: "human",
  ...
  const rescuedStep = {
    id: "rescued-step",
    type: "ai",
    content: "rescued processing step",
  } as Message;
  const newTail = {
    id: "new-tail",
    type: "ai",
    content: "new streaming step",
  } as Message;

  expect(
    stabilizeMessagesByEstablishedOrder(
      [olderHistory, rescuedStep, user, newTail],
      ["message:current-user", "message:rescued-step"],
    ),
  ).toEqual([olderHistory, user, rescuedStep, newTail]);
});

test("stabilizeMessagesByEstablishedOrder preserves array identity when order is already stable", () => {
  const messages = [
    { id: "human-stable", type: "human", content: "question" } as Message,
    { id: "ai-stable", type: "ai", content: "answer" } as Message,
  ];

  expect(
    stabilizeMessagesByEstablishedOrder(messages, [
      "message:human-stable",
      "message:ai-stable",
    ]),
  ).toBe(messages);
});
