...
description: "Interact with AstraFlow AI agent platform via its HTTP API. Use this skill when the user wants to send messages or questions to AstraFlow for research/analysis, start a AstraFlow conversation thread, check AstraFlow status or health, list available models/skills/agents in AstraFlow, manage AstraFlow memory, upload files to AstraFlow threads, or delegate complex research tasks to AstraFlow. Also use when the user mentions deerflow, deer flow, or wants to run a deep research task that AstraFlow can handle."
---

# AstraFlow Skill

Communicate with a running AstraFlow instance via its HTTP API. AstraFlow is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

AstraFlow exposes two API surfaces behind an Nginx reverse proxy:
...
- If health check fails, AstraFlow is not running. Inform the user they need to start it.
