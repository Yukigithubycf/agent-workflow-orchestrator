# AstraFlow Sandbox Provisioner

The **Sandbox Provisioner** is a FastAPI service that dynamically manages sandbox Pods in Kubernetes. It provides a REST API for the AstraFlow backend to create, monitor, and destroy isolated sandbox environments for code execution.
...
When the Gateway mounts that same storage at its AstraFlow home and the PVC
...
If an existing deployment already has PVC-backed user-data under the legacy layout, migrate the AstraFlow data directory before relying on the new PVC subPath. Mount the same PVC path that the gateway uses as its AstraFlow base directory, then run the existing user-isolation migration script:
