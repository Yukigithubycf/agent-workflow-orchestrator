"""AstraFlow Sandbox Provisioner Service.
...
def _host_base_dir_for_extra_mounts() -> str:
    """Return the host-visible AstraFlow state root used for controlled mounts."""
...
def _validated_extra_mounts(extra_mounts: list["ExtraMount"] | None) -> list["ExtraMount"]:
    ...
    for mount in extra_mounts:
        ...
        if not _is_path_under_base(host_path, host_base_dir):
            raise HTTPException(status_code=400, detail=f"Extra mount host path is outside AstraFlow state: {mount.host_path}")
...
def _extra_mount_pvc_sub_path(host_path: str) -> str:
    host_base_dir = _host_base_dir_for_extra_mounts()
    if not _is_path_under_base(host_path, host_base_dir):
        raise HTTPException(status_code=400, detail=f"Extra mount host path is outside AstraFlow state: {host_path}")
...
app = FastAPI(title="AstraFlow Sandbox Provisioner", lifespan=lifespan)
